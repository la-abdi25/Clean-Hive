...
const MessagingDashBoard = () => {
  ...
  //fetches currently logged in nectar user
  useEffect(() => {
      ...
      if (userId) {
        try {
          const res = await axios.get(
            `/api/users/nectar-profile/nectar-data/${userId}`
          );
        } catch (err) {
  ...
  useEffect(() => {
    ...
    //fetches conversations from Firebase
    const getConversationList = async () => {
      ...
      //fetches names from Bee Model that nectar user is conversing with currently
      const findNames = async () => {
        ...
        for (const user of otherUsers) {
          ...
          if (id) {
            const res = await axios.get(
              `/api/users/bee-profile/bee-data/${id}`
            );
            if (res.data.beeData.firstName) {
              data.push({ ...user, firstName: res.data.beeData.firstName });
            }
          }
        }
        return data;
  ...
  // allows nectar to send a message to a bee/customer
  ...
  //points to currently sent message at the bottom of the div
  useEffect(() => {
